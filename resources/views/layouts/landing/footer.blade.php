<footer class="mt-32 bg-primary">
    <div class="mx-auto max-w-lg">
        <div class="flex items-center justify-center px-20 py-5 text-white sm:px-36">
            <div class="relative">
                <h1 class="relative z-50 pr-5 text-xl font-bold">2024</h1>
                <svg class="absolute -left-3 -top-2 z-40 h-11 w-11" viewBox="0 0 79 79" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M35.9645 2.94975C37.9171 0.997129 41.0829 0.997127 43.0355 2.94975L76.0502 35.9645C78.0029 37.9171 78.0029 41.0829 76.0503 43.0355L43.0355 76.0502C41.0829 78.0029 37.9171 78.0029 35.9645 76.0503L2.94975 43.0355C0.997129 41.0829 0.997127 37.9171 2.94975 35.9645L35.9645 2.94975Z"
                        stroke="#26C1F2" stroke-width="2" />
                </svg>
            </div>
            <span class="border-l border-gray-500 py-2 pl-5 text-sm font-semibold">Desa {{ config('app.name') }}</span>
        </div>

        <div class="text-center text-white">
            <div class="py-3 tracking-wide">
                {{-- <p>Made By <span class="font-semibold">RISTI UNG</span></p> --}}
                <p class="font-semibold"> &copy; Made By Gorontalo Software House

                    UI/UX By Irvan Moses

                    © 2024 Gorontalo Software House.</p>
            </div>
        </div>
    </div>
</footer>
